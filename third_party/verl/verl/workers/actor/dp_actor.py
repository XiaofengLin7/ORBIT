...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(
        ...
        with torch.autocast(device_type=self.device_name, dtype=self.param_dtype):
            ...
            if self.use_remove_padding:
                ...
                if self.use_fused_kernels:
                    # fused path doesn't expose logits -> can't compute sum_pi_squared here
                    if calculate_sum_pi_squared:
                        raise NotImplementedError(
                            "calculate_sum_pi_squared requires logits, but use_fused_kernels=True path "
                            "does not provide logits. Disable fused kernels or extend fused output."
                        )
                    ...
                    # >>> added: sum_pi_squared on rmpad logits
                    if calculate_sum_pi_squared:
                ...
                if self.use_fused_kernels:
                    ...
                    # >>> added: sum_pi_squared on dense logits
                    if calculate_sum_pi_squared:
                        if not sum_pi_squared_checkpointing:
    ...
    def compute_log_prob(self, data: DataProto, calculate_entropy=False) -> dict[str, torch.Tensor]:
        ...
        for micro_batch in micro_batches:
            ...
            if calculate_sum_pi_squared:  # >>> added
